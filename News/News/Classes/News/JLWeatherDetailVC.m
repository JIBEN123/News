...
@implementation JLWeatherDetailVC
...
- (void)viewDidLoad {
    ...
    [self addWeather];
    for (int i = 1 ; i < 4 ; i++) {
        JLWeatherDetailM *weatherDetail = self.weatherModel.detailArray[i];
...
{
    JLWeatherItemView *itemView = [JLWeatherItemView view];
    itemView.width = W/3;
    itemView.y = 0;
    itemView.height = 200;
...
- (void)addWeather
{
    ...
    JLWeatherDetailM *weatherDetail = self.weatherModel.detailArray[0];
