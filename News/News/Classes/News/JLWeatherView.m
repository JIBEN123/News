...
- (void)setWeatherModel:(JLWeatherModel *)weatherModel
{
    _weatherModel = weatherModel;
    self.nowTempLbl.text = [NSString stringWithFormat:@"%d",weatherModel.rt_temperature];
    JLWeatherDetailM *weatherDetail = weatherModel.detailArray[0];
...
+ (instancetype)view{
    return [[NSBundle mainBundle]loadNibNamed:@"JLWeatherView" owner:nil options:nil].firstObject;
}
